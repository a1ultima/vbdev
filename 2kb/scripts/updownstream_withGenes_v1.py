...
def sampleSequences_read ( ensembl_genome, sample_direction='upstream', sample_range=2000, sample_data={}, sample_seqs={}):
    ...
    for geneId in geneIds:
        ...
        if sample_direction=='upstream':
            sampleLocation  = geneLocation.resized(-sample_range-1,-len(geneLocation)-1) # focus on the sampled region, #TODO: check if off by 1 @@@@@@@@@@@
        elif sample_direction=='downstream':
            sampleLocation  = geneLocation.resized(+len(geneLocation)+1,+sample_range+1) 
        ...
        if not overlap_features:            # keep full sample & skip to next loop if no features are found
            ...
            #
            overlap_locations = [i for i in overlap_locations if (i.Start < sampleLocation.End) and (i.End > sampleLocation.Start)]
            
            o2 = overlap_locations
...
o1, o2, o3, gene, sampleLocation_truncated, sampleLocation  = sampleSequences_read (    genome, sample_direction='downstream', sample_range=200, sample_data={}, sample_seqs={})

# genome          = setupGenome(              'Anopheles gambiae', db_host='localhost', db_user='vbuser', db_pass='Savvas', db_release=73 )
# samples_read    = sampleSequences_read (    genome, sample_direction='downstream', sample_range=200, sample_data={}, sample_seqs={})
# samples_write   = sampleSequences_write(    genome, samples_read, fasta_it=True, pickle_it=False, include_genes=False)
