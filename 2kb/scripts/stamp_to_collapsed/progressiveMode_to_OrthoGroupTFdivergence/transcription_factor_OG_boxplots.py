...
def paralogue_check():
    ...
    Performs a recursive user input check, asking if OGs to boxplot should have paralogue copies >1 or not.

    """
    paralogues = raw_input('paralogues included? (y/n)')
    if str(paralogues) == 'y':
        return True 
    elif str(paralogues) == 'n':
        return False
    else:
        print 'Incorrect input, please type y or n then press return...'
        paralogue_check()
...
def boxplot_tfs(data1,data2,labels,save):
    ...
    #########################################
    # First Y-axis: Boxplots of %id OG TFs 
    #########################################
    ...
    ########################################
    # Second Y-axis: Number of TFs present
    ########################################
    ...
    #plt.gcf().set_size_inches(19.5,10.5)  # resize the plot to make it pretty # @original
    plt.gcf().set_size_inches(30,10.5)  # resize the plot to make it pretty # @testing
...
def get_tf_divergences(paralogues):

    """

    """

    # Do we include OGs who have duplications of any of hteir members? I.e. paralogue counts >1?

    # NOTE: bitscoreVecs and bitscoreVecs_noZeros are obtained by line: CTRL+F: "bitscoreVecs,bitscoreVecs_noZeros = pickle.load( f )"

    if paralogues:
        print 'OGs DO have at least one duplication (OGs DO have paralogue counts >1)'
        savename_suffix = '_incl_paralogues'
    else:
        print 'OGs DO NOT have duplications (OGs do NOT have paralogue counts >1)'
        savename_suffix = '_excl_paralogues'
    ...
    try:
        ...
        f = open( './data_in_transcription_factor_OG_boxplots'+savename_suffix+'.p' )
        ...
        return bitscoreVecs,bitscoreVecs_noZeros,savename_suffix

        # No: then generate it
    except IOError as detail:

        print detail
        print'..therefore we shall now generate it, please wait ~1 minute...'
        ...
        line    = fi.readline()     # first line are headers to specifnes
        row     = line.split(',')
        #species = [j.replace('\r\n','') for j in row]
        ...
        while True:
            ...
            if mz in transcription_factor_mzIds:
                ...
                # "WITH PARALOGUES":    Note: this is mutually exclusive w/ "NO PARALOGUES"
                elif paralogues == True:
                    if len([i for i in pc if i >1])>0: 
                        transcription_factor_mzIds_with_no_paralogues.append(mz) #..then list it!
        fi.close()
        ...
        f = open('data_in_transcription_factor_OG_boxplots'+savename_suffix+'.p','wb')
...
paralogues = paralogue_check() # e.g. True // Performs a recursive user input check, asking if OGs to boxplot should have paralogue copies >1 or not.

bitscoreVecs,bitscoreVecs_noZeros,savename_suffix = get_tf_divergences(paralogues)
...
save_wZeros    = './boxcompare_TF_AGAP_OG_percId_wZeros'+savename_suffix+'.png'
...
save        = './boxcompare_TF_AGAP_OG_percId_noZeros'+savename_suffix+'.png'
debug2 = boxplot_tfs(data1,data2,labels,save)


print 'Proportion of TFs w/ medians > threshold: '

nOverThresh_wZeros = str(100*(float(len([mean for (mz,vec,mean,ag,nTfs) in bitscoreVecs if mean > 70]))/float(len([mean for (mz,vec,mean,ag,nTfs) in bitscoreVecs]))))+'%'

nOverThresh_noZeros = str(100*(float(len([mean for (mz,vec,mean,ag,nTfs) in bitscoreVecs_noZeros if mean > 70]))/float(len([mean for (mz,vec,mean,ag,nTfs) in bitscoreVecs_noZeros]))))+'%'


print '\tZeroes removed from boxplot: '+str(nOverThresh_noZeros)

print '\tZeros kept in boxplot: '+str(nOverThresh_wZeros)