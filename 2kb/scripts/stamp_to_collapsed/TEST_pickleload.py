...
def rename_blacklist( blacklist, version = 1 ):
    ...
    for n,(c,d) in enumerate(blacklist):   # e.g. (c,d) = ('c397_n002_CTGATGGSC','0.056')
        ...
        fbp_path = cluster_to_stats['e0.05_d'+str(d)][c]['cluster']['path']['fbp'] # e.g. '../data/stamp_data/out/dreme_100bp_e0.05/SWU_SSD/cluster_motifs_d0.056/family_binding_matrices/c397_n002_CTGATGGSCFBP.txt'
...
def blacklist_then_summaryStats_from_shell( e = 0.05, species_threshold = 3.0, entropy_threshold = 1.0,cluster_to_stats=None ):
    ...
    cd_to_vbname = rename_blacklist( blacklist, version = 1 ) # dictionary: converts a blacklisted motif cluster (c,d) into a vectorbase name e.g. of usage: vbname = cd_to_vbname[(c,d)] // e.g. of vbname = 'MM10119_CGATGCGAT_ATCGCATCG' // general format: MM<version><id, e.g. 0001>_<STAMP ambiguity seq>_<reverse complement>..
...
moo = blacklist_then_summaryStats_from_shell( e = 0.05, species_threshold=species_threshold, entropy_threshold=entropy_threshold, cluster_to_stats=cluster_to_stats )