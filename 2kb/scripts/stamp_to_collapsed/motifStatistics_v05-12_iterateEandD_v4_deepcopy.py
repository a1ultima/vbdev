...
parameterisation_events = [(0.05,0.05),(0.05,0.5)]   # parameterisations >>> ( evalue, dvalue )
#parameterisation_events = [(0.05,0.5)]   # parameterisations to draw data from
...
species_to_index= {} # species now point to the indices of a list which will later be the vector of counts and presence/absence for phylo profiles
...
for e,save_d in parameterisation_events:
    ...
    for count,cluster_filename in enumerate(clusters_files):
        ...
        for count_sp,species in enumerate(species_list):
            cluster_to_stats[cluster]['species']['phy_profile']['binary']['dict_mode'][species] = 0
            cluster_to_stats[cluster]['species']['phy_profile']['count']['dict_mode'][species]  = 0
    ...
    # AVERAGE MOTIF LENGHT
        cluster_to_stats[cluster]['motif']['avg_length'] = np.array(motif_lenghts).mean() # average the lengths of motifs

    # append the cluster to species who were found in the cluster
        for count_sp,species in enumerate(cluster_species):
            cluster_to_stats[cluster]['species']['phy_profile']['binary']['dict_mode'][species] = 1
            cluster_to_stats[cluster]['species']['phy_profile']['count']['dict_mode'][species]  = 1 + cluster_to_stats[cluster]['species']['phy_profile']['count']['dict_mode'][species]

        for s in cluster_to_stats[cluster]['species']['phy_profile']['binary']['dict_mode'].keys():
            
            index   = species_to_index[s]
            binary  = cluster_to_stats[cluster]['species']['phy_profile']['binary']['dict_mode'][s] # presence/absence of species s in the dict
            count1  = cluster_to_stats[cluster]['species']['phy_profile']['count']['dict_mode'][s] # counts of ^^ of species 

            cluster_to_stats[cluster]['species']['phy_profile']['binary']['list_mode'][index]   = binary
            cluster_to_stats[cluster]['species']['phy_profile']['count']['list_mode'][index]    = count1
    ...
    cluster_names       = cluster_to_stats.keys()

    # Numbers of motifs (column 2)
    cluster_nMotifs     = [cluster_to_stats[i]['motif']['n'] for i in cluster_to_stats.keys()]

    # Entropy of clusters ()
    cluster_entropies   = [cluster_to_stats[i]['cluster']['H'] for i in cluster_to_stats.keys()]

    # Numbers of unique species ()
    cluster_nSpecies   = [cluster_to_stats[i]['species']['unique']['n_unique'] for i in cluster_to_stats.keys()]

    # Numbers of paralogues ()
    cluster_nSpecies   = [cluster_to_stats[i]['species']['unique']['n_paralogue'] for i in cluster_to_stats.keys()]

    #   PHYLOGENETIC PROFILE:

    # Phylo profile - binary ()
    cluster_nSpecies   = [cluster_to_stats[i]['species']['phy_profile']['binary']['list_mode'] for i in cluster_to_stats.keys()]

    # Phylo profile - count ()
    cluster_nSpecies   = [cluster_to_stats[i]['species']['phy_profile']['count']['list_mode'] for i in cluster_to_stats.keys()]
