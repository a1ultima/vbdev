...
# Events as primary key
event_to_clusterClass = {
    'e0.05_d0.05':{},
    'e0.5_d0.05':{}
    }
...
for event in event_to_clusterClass.keys():
    ...
    event_to_clusterClass[event] = event_to_clusterClass_template # cluster classes

    # all               :   summary stats for all the clusters
    clusters_all                                                = cluster_to_stats.keys()
    event_to_clusterClass[event]['all']                         = clusterClass_to_stats_template
    event_to_clusterClass[event]['all']['list']                 = clusters_all
    event_to_clusterClass[event]['all']['nClusters']            = len(clusters_all)
    event_to_clusterClass[event]['all']['H']['distribution']    = np.array([])
    event_to_clusterClass[event]['all']['H']['mean']            = float()
    ...
    for s in range(1,len(species_list)): # per species number, S
        ...
        clusters_s_entropy = np.array([cluster_to_stats[c]['cluster']['H'] for c in clusters_s])

        event_to_clusterClass[event]['nSpeciesOrMore'][s]                       = clusterClass_to_stats_template
        ...
        event_to_clusterClass[event]['nSpeciesOrMore'][s]['H']['mean']          = float(clusters_s_entropy.mean())
    ...
    for clade in event_to_clusterClass_template['clades'].keys():
        event_to_clusterClass[event]['clades'][clade] = clusterClass_to_stats_template # append common data template to each clade
    ...
    event_to_clusterClass[event]['clades']['inc_DMEL']['H']['distribution']  = np.array([])
    event_to_clusterClass[event]['clades']['inc_DMEL']['H']['mean']          = float()
...
# event_to_clusterClass['e0.05_d0.05']

# event_to_clusterClass['e0.05_d0.05']['all'] = {
#     'list':         clusters_all,
...
#     # append to event_to_clusterClass dict
#     if not event_to_clusterClass['e0.05_d0.05'].has_key('nSpeciesOrMore'):
#         event_to_clusterClass['e0.05_d0.05'] = {'nSpeciesOrMore':{s:{'list':clusters_s,'nClusters':len(clusters_s),'H':{'distribution':[],'mean':float()}}}}
#     else:
#         event_to_clusterClass['e0.05_d0.05']['nSpeciesOrMore'][s] = {'list':     clusters_s, # same as above but once the 'nSpeciesOrMore' key is available
...
# # if not event_to_clusterClass['e0.05_d0.05'].has_key('inc_DMEL'):
# #     event_to_clusterClass['e0.05_d0.05'] = {'inc_DMEL':{ 'list':clusters_dmel,
# #                                                 'nClusters':len(clusters_dmel),
# #                                                 'H':float()}}    

# event_to_clusterClass['e0.05_d0.05']['inc_DMEL'] = { 
#     'list':     clusters_dmel,
#     'nClusters':len(clusters_dmel),
#     'H':   float()
#     }
...
# for cluster_set in event_to_clusterClass['e0.05_d0.05'].keys():
    
#     clusters            = event_to_clusterClass['e0.05_d0.05'][cluster_set]['list']
