...
def exploreCutoffs( dataPath, filename_in, d_from = 0, d_to = 2, d_step = 0.05, save_d = 0.10):
    ...
    if not str(save_d) in str(np.arange(d_from,d_to,d_step)):
        raise Exception(" savde_d does not lie in the distance-cut-off range!! this is asking for trouble... you won't get your tree saved... look: "+"Tree Range: "+str(np.arange(d_from,d_to,d_step)))
...
# Statistics and data vars
    ...
    for count,d in enumerate(distances):     # explore distance threshold params
        ...
        # CLUSTER NAMES, HEADERS_FOR_FASTA  :   saves on a specified collapsing distance cut-off, save_d
        
        if d in save_d: # @SAVED
...
d_vec = np.arange(0.3,0.5,0.05)

for distance in d_vec:
    ...
    try:
        ...
        choice = raw_input('data is already available in: '+dataPath_clusters_out+' delete? (y/n)')
        if choice == 'y':
            import shutil
    ...
    # if os.path.exists(dataPath_clusters_in):
    #     print(' Generating file system... fasta  --> as input for stamp --> to generate cluster FBPs')
    #     os.makedirs(dataPath_clusters_in) # @ANDY_05-19 this can be bad, put it here since headers_for_fasta was not present
    #     filename_clusters_list = os.listdir(dataPath_clusters_in) # list the collapsed motif files to iterate over

    # if not filename_clusters_list :     # TEST : empty file list
    #     choice = raw_input('maybe remove the dataPath_in and out then try again? (y/n)')
    #     if choice == 'y':
    #         import shutil
    #         shutil.rmtree(dataPath_clusters_in)
    #         shutil.rmtree(dataPath_clusters_out)
    #     raise Exception(' Now run the script again! ')
