...
for e,save_d in parameterisation_events:
    ...
    for count,cluster_filename in enumerate(clusters_files):
        ...
        cluster_to_stats[cluster] = OrderedDict()
    ...
    for event in event_to_clusterClass.keys():
        ...
        event_to_clusterClass[event] = event_to_clusterClass_template.deepcopy() # cluster classes
        ...
        event_to_clusterClass[event]['all']                         = clusterClass_to_stats_template.deepcopy()
        ...
        for s in range(1,len(species_list)+1): # per species number, S
            ...
            event_to_clusterClass[event]['nSpeciesOrMore'][s]                       = clusterClass_to_stats_template.deepcopy()
        ...
        for clade in event_to_clusterClass_template['clades'].keys():
            event_to_clusterClass[event]['clades'][clade] = clusterClass_to_stats_template.deepcopy() # append common data template to each clade
