...
def exploreCutoffs( dataPath, filename_in, d_from = 0, d_to = 2, d_step = 0.05, save_d = 0.10):
    ...
    # TEST  :   catch annoying errors where the cutting range does not include the save_d !!!!

    if not save_d in np.arange(d_from,d_to,d_step):
        raise Exception(" savde_d does not lie in the tree-cutting range!! this is asking for trouble... you won't get your tree saved :( ")
...
try: 
    ...
    print('check the tree name and path are correct...')
    #
    #   MAIN PROGRAM HERE >>>>>>
    #
    #
...
if not os.path.exists(dataPath_clusters_out): # generate cluster fasta dirs
    print(' Generating file system... headers_for_fasta --> as input to pull out fastas ')
    os.makedirs(dataPath_clusters_out)

# if not os.path.exists(dataPath_clusters_in):
