...
def sampleSequences_read( ensembl_genome, sample_direction='upstream', sample_range=2000, sample_data={}, sample_seqs={}):
    ...
    # INITIATE STORAGE:         ANDY: concatenate just the limiting exon sequence to the sampleseq?
        sample_seqs[geneId] = {'untruncated':str(),'truncated':str(),'gene_seq':str(gene_limiting_exon.Seq),'sample_location':str(),'gene_location':':'.join(str(gene_limiting_exon.Location).split(':')[2:])}
...
def sampleAllSpecies(species_list,sample_directions):
    ...
    for sample_direction in sample_directions:
        for species in species_list:
            ...
            samples_read    = sampleSequences_read (    genome, sample_direction=direction, sample_range=200, sample_data={}, sample_seqs={})
...
species_list = [    'Aedes aegypti',
                ...
                'Anopheles sinensis',
                #'Anopheles stephensiI',        # this is brocken..?
                'Anopheles stephensi']

sampleAllSpecies(species_list)
