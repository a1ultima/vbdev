...
Description: 
    ...
    Remove duplicate rows present in the a_collections.txt isatab sheets generated by isatabGen*.py. 

    Warning: duplicated rows may not always be in the same order every time this script is run. This is because it involves a step that involves iterating through the keys of a dict, and the keys of a dict are not ordered bespokely (but presumably by some random process when the dict is created).
...
# collection
    ...
    for primaryKey in data['a_collection'].keys():
        ...
        duplicatedAssayName = uniqueSample.split('\t')[1]
        dataRow             = ''.join(['\t'.join([i.split('\t')[0],duplicatedAssayName+'\t'+'\t'.join(primaryKey)+'\n']) for i in data['a_collection'][primaryKey]]) #04-25 we need to use the sampleName (i.e. MAP.1069) but duplicate the assayName (i.e.MAP.1069.collection)
        #                                   ^Changing SampleName ^Constant AssayName (machine dupe) ^Duped row data   ^List of Sample-Assay Names sharing the duped row data 
        file_out_collection.write(dataRow)
...
#         primaryKey  = (description,protocol1,protocol2,species,term,accession,assi) # bunches the column elements above, into a single data row
...
#         duplicatedAssayName = uniqueSample.split('\t')[1]
#         dataRow             = ''.join(['\t'.join([i.split('\t')[0],duplicatedAssayName+'\t'+'\t'.join(primaryKey)+'\n']) for i in data['a_species'][primaryKey]]) #04-25 we need to use the sampleName (i.e. MAP.1069) but duplicate the assayName (i.e.MAP.1069.species)
#         #                                   ^Changing SampleName ^Constant AssayName (machine dupe) ^Duped row data   ^List of Sample-Assay Names sharing the duped row data 
#         file_out_species.write(dataRow)
