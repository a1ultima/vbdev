...
filename_in = dataPath_onto+'species'
...
for continent in continents:
    print('\t'+continent)
    # READ VECTORIZED DATA      :       Columns of data that will be divided into: s_samples, a_collection, a_species
    ...
    try:
        ...
        print('\t\tAccumulating methods: i_investigations...WHY NAME ERROR')
...
#===============#
    ...
    # ISA-TAB COLUMNS: ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    #                       |                       |                       |               |                   |                   |                       |                                   |                                                                       |
    headers     =           'Source Name\t          Sample Name\t           Description\t   Material Type\t     Term Source REF\t   Term Accession Number\t Source\t                            Comment [PubMed ID]\n'                                                
    values      = ''.join([ 'malariaAtlas'+'\t'+    'MAP.'+sampleName+'\t'+ ''+'\t'+        'population'+'\t'   'OBI'+'\t'+         '0000181'+'\t'+         citations[i].replace('"','')+'\t'+  citation_raw_to_pubmed[citations[i].replace('"','')]['id_pubmed']+'\n'  for i,sampleName in enumerate(sampleNames)]) 
    #                       |                       |                       |               |                   |                   |                       |                                   |                                                                       |
    #----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
