...
def diff(path1,path2):
    ...
    Description:
        ...
        Compare differences in lines of two separate files where the order of lines do not matter. I.e. Differences are not made with reference to the two files being aligned rowise, instead lines that are different between two files are pinted out.
        All lines from path1 are prefixed with <, and lines from path2 are prefixed with >.

    Arguments:

        path1       # path of the first file to be compared
        path2       # path of the second file to be compared
...
def main(argv):
    ...
    for opt, arg in opts:
        ...
        if opt == '-h': # help file                                                 \/-- @@@
            ...
            f = str(arg) # don't forget to correct the type!!                             <-- @@@
